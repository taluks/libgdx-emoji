...
public class ExtendedFreeTypeGlyphLayout extends GlyphLayout {
    private final Array<Color> colorStack = new Array<>(4); 	
	
    public ExtendedFreeTypeGlyphLayout() {
		super();
	}

	public ExtendedFreeTypeGlyphLayout(BitmapFont font, CharSequence str, Color color, float targetWidth, int halign,
			boolean wrap) {
		super(font, str, color, targetWidth, halign, wrap);
	}

	public ExtendedFreeTypeGlyphLayout(BitmapFont font, CharSequence str, int start, int end, Color color,
			float targetWidth, int halign, boolean wrap, String truncate) {
		super(font, str, start, end, color, targetWidth, halign, wrap, truncate);
	}

	public ExtendedFreeTypeGlyphLayout(BitmapFont font, CharSequence str) {
		super(font, str);
	}
	...
	public void setText (BitmapFont font, CharSequence str, int start, int end, Color color, float targetWidth, int halign,
    		boolean wrap, String truncate) {
		 	...
		 	ArrayMap<Integer, Character> codeMap = ((ExtendedFreeTypeBitmapFont)font).surrogateCodeMap;
    		...
    		while (true) {
    			...
    			if (runEnd != -1) {
    				...
    				if (runEnd != runStart) { // Eg, when a color tag is at text start or a line is "\n".
    					...
    					temp.setLength(runStart);
    					int index = runStart;
    					int offset = 0;
    					while(index < runEnd) {
    						int codePoint = ((String)str).codePointAt(index);
